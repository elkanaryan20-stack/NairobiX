...
function validateForm(formType: string, data: Record<string, unknown>) {
  ...
  if (missing.length > 0) {
    return { valid: false, error: `Please complete the required fields: ${missing.join(", ")}.` };
  }
  ...
  if (website && !URL_REGEX.test(website)) {
    return { valid: false, error: "Please enter a valid website URL, including http:// or https://." };
  }
  ...
  if (formType === "business-growth-audit") {
    ...
    if (channels.includes("None currently") && channels.length > 1) {
      return { valid: false, error: "'None currently' cannot be selected alongside other marketing channels." };
    }
  }
  ...
  if (formType === "request-solution") {
    ...
    if (solutions.includes("Custom Quote") && solutions.length > 1) {
      return { valid: false, error: "'Custom Quote' must be selected on its own if you'd like to request a custom project plan." };
    }
  }
...
export async function POST(request: Request) {
  try {
    ...
    if (!VALID_FORM_TYPES.includes(formType as (typeof VALID_FORM_TYPES)[number])) {
      return NextResponse.json({ success: false, error: "Unsupported form type." }, { status: 400 });
    }

    const validation = validateForm(formType, json as Record<string, unknown>);
    if (!validation.valid) {
      return NextResponse.json({ success: false, error: validation.error }, { status: 400 });
    }

    const payload = mapLeadPayload(formType, json as Record<string, unknown>);
    ...
    if (!result.ok) {
      ...
      return NextResponse.json(
        { success: false, error: "We couldn't submit your request right now. Your information hasn't been lost. Please try again." },
        { status: 500 }
      );
    }

    console.log("Zoho CRM submission successful", { formType, leadId: result.data?.data?.[0]?.id });
