...
export function BusinessGrowthAuditForm() {
  ...
  const validateForm = () => {
    ...
    if (formData.Current_Marketing_Channels.length === 0) {
      nextErrors.Current_Marketing_Channels =
        "Please select at least one option.";
    }

    if (
      formData.Current_Marketing_Channels.includes(
        "None currently"
      ) &&
      formData.Current_Marketing_Channels.length > 1
    ) {
      nextErrors.Current_Marketing_Channels =
        "'None currently' must be selected on its own.";
    }
      ...
      >
        ...
        <div>
          <SectionHeader
            ...
            description="Understanding your current channels helps us identify the fastest route to better growth."
          />
